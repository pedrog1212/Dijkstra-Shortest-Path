...
#include "Graph.hpp"
    ...
    // Destructor
    Graph::~Graph() {
        for (auto vertex : vertices) {   // delete all Vertex instances from memory
            delete vertex;
        }
        
        for (auto edge : edges) {       // delete all Edge instances from memory
           delete edge;    
        }
    ...
    void Graph::addVertex( string label) {
        ...
        if (index != -1 ) throw invalid_argument("Vertex already exists");  // throw exception "Vertex already exists"

        vertices.push_back(new Vertex(label));          // Add the new instance of the Vertex class to vertices list
...
/**
    ...
    void Graph::removeVertex(string label) {
        ...
        for (auto& vertex : vertices)
            try {
                ...
                continue;   // handle the exception to continue finding incident edges of vertex
            }
        
        // Remove the vertex
        delete vertices[index];                             // deallocate the memory of this Vertex instance (created with new)
        
        vertices.erase(vertices.begin() + index);           // remove the vertex from Vertices using iterator
        
        adjacencyList.erase(adjacencyList.begin() + index); // remove vertex from the adjacency list

        // Adjust adjacency list indexes, substracting 1, from the indexes larger than the removed vertex
    ...
    void Graph::addEdge( string label1, string label2, unsigned long weight) {
        ...
        // Add the edge to the adjacency list because the graph is undirected
        adjacencyList[index1].push_back(index2);   
        
        adjacencyList[index2].push_back(index1);
        
        edges.push_back(new Edge(label1, label2, weight));  // create an instance of Edge and add it to the edges list
    }
    ...
    void Graph::removeEdge( string label1,  string label2) {
        ...
        // Remove the edge from adjacency list of index1 to index2 using an iterator
        ...
        neighbors1.erase(it2);  // In the vertex label1, erase the neighbor with label2

        // Remove the edge from adjacency list of index2 to index1 using an iterator
        auto& neighbors2 = adjacencyList[index2];
        auto it1 = find(neighbors2.begin(), neighbors2.end(), index1); // in vertex label2, find the neighbor with label1 

        neighbors2.erase(it1);  // In the vertex label2, erase the neighbor with label1
    ...
    unsigned long Graph::shortestPath( string startLabel, string endLabel, vector<string>& path) {
        ...
        vector<unsigned long> distances (vertices.size(), numeric_limits<unsigned long>::max()); // vector initialized with ULONG_MAX to compare distances (infinite)
        
        // Previous vector: stores the previous vertex indexes in the shortest path
        vector<int> previous (vertices.size(), -1);
    
        // Implementation of a lambda fuction as comparator for PQ. PQ implemented over a Min-Heap
